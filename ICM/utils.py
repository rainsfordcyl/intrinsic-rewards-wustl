...
def setup_mpi_gpus():
    ...
    if not available_gpus:
        raise Exception("No available GPUs found")
    ##
    ...
    if local_rank >= len(available_gpus):
        raise Exception(f"Local rank {local_rank} exceeds number of available GPUs {len(available_gpus)}")
    ##

    os.environ['CUDA_VISIBLE_DEVICES'] = str(available_gpus[local_rank])

    ## added
    print(f"Process {MPI.COMM_WORLD.Get_rank()} using GPU {available_gpus[local_rank]} on node {node_id}")
    ##
...
#     selected_gpu = available_gpus[local_rank % len(available_gpus)]

#     os.environ['CUDA_VISIBLE_DEVICES'] = str(selected_gpu)

#     print(f"Process {MPI.COMM_WORLD.Get_rank()} using GPU {selected_gpu} on node {node_id}")

#     # TensorFlow configuration to allow memory growth on the GPU
...
def setup_tensorflow_session():
    ...
    if tf.test.is_gpu_available():  # Check if GPU is available

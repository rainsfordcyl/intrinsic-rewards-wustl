...
class CnnPolicy(object):
    ...
    def get_ac_value_nlp(self, ob):
        # ob.shape = (128, 84, 84, 4), type(ob) = np.ndarray
        a, vpred, nlp = \
    ...
    def get_action(self, ob):
        print(ob.shape)
        print(type(ob))
        trained_checkpoint_prefix = './checkpoints/checkpoint.chk'
        loaded_graph = tf.Graph()
        with tf.Session(graph=loaded_graph) as sess:
            # Restore from checkpoint
            loader = tf.train.import_meta_graph(trained_checkpoint_prefix + '.meta')
            loader.restore(sess, trained_checkpoint_prefix)
            ph = tf.placeholder(dtype=tf.int32, shape=(1, 84, 84, 4), name='ob')
            a, vpred, nlp = sess.run([self.a_samp, self.vpred, self.nlp_samp], feed_dict={ph: ob})
        return a[:, 0]

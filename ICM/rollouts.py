...
class Rollout(object):
    ...
    def calculate_reward(self):
        ...
        int_rew = self.dynamics.calculate_loss(ob=self.buf_obs,
                                               ...
                                               acs=self.buf_acs)
        self.buf_int_rews[:] = int_rew  # store intrinsic rewards
        # combines intrinsic reward and extrinsic rewards
    ...
    def rollout_step(self):
        ...
        for l in range(self.nlumps):
            ...
            obs_with_batch = obs.reshape((obs.shape[0], 1, *obs.shape[1:]))
            next_obs = self.buf_obs_last[sli, t // self.nsteps_per_seg].reshape((obs.shape[0], 1, *obs.shape[1:]))  # reshape last_ob
            acs_reshaped = acs.reshape(-1, 1)  # Ensure acs is reshaped to 2D tensor
            
            intrinsic_reward_value = getsess().run(self.dynamics.intrinsic_reward, feed_dict={
                self.dynamics.obs: obs_with_batch,
                self.dynamics.last_ob: next_obs,  # feed last_ob placeholder
                self.policy.ph_ac: acs_reshaped
            })
            intrinsic_reward_value = intrinsic_reward_value.reshape(-1)
            self.int_rew[sli] = intrinsic_reward_value
